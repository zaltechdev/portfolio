...
<div class={cn('flex flex-col gap-1', containerWidth)}>
	...
	<div class={cn(
		'relative flex rounded-lg border bg-transparent transition-colors duration-300',
		baseBorder,
		disabled ? 'opacity-50 cursor-not-allowed' : ''
	)}>
		{#if iconLeft}
			<div class="absolute left-0 top-0 flex items-center justify-center w-10 h-10 text-neutral-400 pointer-events-none">
				<i class={iconLeft}></i>
			</div>
		{/if}
		...
		<textarea
			bind:value
			class={cn(
				'w-full min-h-5rem py-2 bg-transparent outline-none text-neutral-50 resize-y',
				paddingLeft,
				paddingRight,
				className
			)}
		...
		{#if iconRight}
			<div class="absolute right-0 top-0 flex items-center justify-center w-10 h-10 text-neutral-400 pointer-events-none">
				<i class={iconRight}></i>
			</div>
		{/if}
	</div>
