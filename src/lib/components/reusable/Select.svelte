...
<div class={cn('flex flex-col gap-1', containerWidth)}>
	...
	<div class={cn(
		...
		{#if iconLeft}
			<div class="absolute left-0 flex items-center justify-center w-10 h-10 text-neutral-400 pointer-events-none">
				<i class={iconLeft}></i>
			</div>
		{/if}
		...
		<select
			bind:value
			class={cn(
				'w-full h-full bg-transparent outline-none text-neutral-50 appearance-none',
				paddingLeft,
				paddingRight,
				className
			)}
		...
		<!-- Default Select Chevron if no right icon is provided -->
		{#if iconRight}
			<div class="absolute right-0 flex items-center justify-center w-10 h-10 text-neutral-400 pointer-events-none">
				<i class={iconRight}></i>
			</div>
		{:else}
			<div class="absolute right-0 flex items-center justify-center w-10 h-10 text-neutral-400 pointer-events-none">
