...
<Modal bind:open={addModalOpen} title="Tambah Projek Baru" class="max-w-xl">
    <form
        ...
        class="flex flex-col gap-5 max-h-[75vh] overflow-y-auto pr-2"
    >
        <!-- Photo Upload -->
        <div class="flex flex-col gap-2">
            <span class="text-sm text-neutral-300">Mockup / Banner Projek</span>
            <div
                class="w-full aspect-video rounded-xl overflow-hidden border border-neutral-800 bg-neutral-950 flex items-center justify-center relative shadow-lg"
            >
                {#if createPhotoPreview}
                    <Image
                        src={createPhotoPreview}
                        alt="Preview Projek Baru"
                        ratio="landscape"
                        objectPosition="top"
                    />
                {:else}
                    <div
                        class="flex flex-col items-center justify-center text-neutral-600 gap-2"
                    >
...
<Modal bind:open={editModalOpen} title="Edit Projek" class="max-w-xl">
    {#if selectedProject}
        ...
        >
            ...
            <div
                class="border-t border-neutral-800 pt-5 flex justify-end gap-3"
            >
                <Button
                    type="button"
                    variant="secondary"
    ...
    {#if selectedProject}
        <div class="flex flex-col gap-4">
            <p class="text-sm text-neutral-300">
                Apakah Anda yakin ingin menghapus projek <span
            ...
            >
                ...
                <Button
                    type="submit"
                    class="bg-red-600 border-red-500 hover:bg-red-700 active:bg-red-800"
                >
                    Hapus
                </Button>
            </form>
...
<div class="flex flex-col gap-6">
    <!-- Page Header -->
    <div
        class="border-b border-neutral-800 pb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"
    >
        <div>
            <h1
                class="text-2xl font-extrabold text-neutral-100 flex items-center gap-2"
            >
                <i class="ri-folders-line text-indigo-400"></i> Kelola Daftar Projek
            </h1>
            <p class="text-neutral-400 text-sm mt-1">
                Tambahkan, ubah, atau hapus karya projek portofolio Anda di
                sini.
            </p>
        </div>
    ...
    {#if !data.profileExists}
        <!-- Warning State -->
        <div
            class="bg-amber-950/20 border border-amber-800/40 rounded-2xl p-8 flex flex-col items-center text-center gap-4 max-w-xl mx-auto mt-8"
        >
            <div
                class="w-12 h-12 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-amber-400"
            >
                <i class="ri-alert-line text-xl"></i>
            </div>
            <div class="flex flex-col gap-1">
                <h3 class="text-lg font-bold text-neutral-100">
                    Profil Owner Belum Dibuat
                </h3>
                <p class="text-neutral-400 text-sm">
                    Anda harus melengkapi profil owner terlebih dahulu sebelum
                    bisa mengelola projek.
                </p>
            </div>
    ...
    {:else if data.projects.length === 0}
        <!-- Empty Projects -->
        <div
            class="bg-neutral-900 border border-neutral-800 rounded-2xl p-12 flex flex-col items-center justify-center text-center gap-4 max-w-xl mx-auto mt-8"
        >
            <div
                class="w-14 h-14 rounded-2xl bg-neutral-950 border border-neutral-800 flex items-center justify-center text-neutral-500"
            >
            ...
            <div class="flex flex-col gap-1">
                ...
                <p class="text-neutral-500 text-sm">
                    Daftar projek Anda kosong. Klik tombol **Tambah Projek** di
                    atas untuk menambahkan projek pertama Anda!
                </p>
            </div>
        </div>
    {:else}
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            {#each data.projects as project}
                <div
                    class="bg-neutral-900 border border-neutral-800 rounded-2xl p-5 flex flex-col gap-4 hover:border-neutral-700 transition-all duration-300"
                >
                    <!-- Project Banner -->
                    <div
                        class="w-full aspect-video rounded-xl overflow-hidden border border-neutral-800 bg-neutral-950 flex items-center justify-center relative"
                    >
                        {#if project.photoUrl}
                            <Image
                        ...
                        {:else}
                            <div
                                class="flex flex-col items-center justify-center text-neutral-600 gap-1"
                            >
                    ...
                    <div class="flex flex-col gap-2 flex-1">
                        <h3
                            class="text-base font-bold text-neutral-100 truncate"
                        >
                            {project.title}
                        </h3>
                        ...
                        <div class="flex flex-wrap gap-1.5">
                            {#each project.techstack
                                .split(",")
                                .map((t) => t.trim())
                                .filter(Boolean) as tag}
                                <span
                                    class="px-2 py-0.5 rounded bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 font-semibold text-[10px] uppercase"
                                >
                                    {tag}
                                </span>
                            {/each}
                        ...
                        <p
                            class="text-neutral-400 text-xs mt-1 leading-relaxed line-clamp-3 whitespace-pre-line"
                        >
                            {project.description}
                        </p>
                    </div>
                    ...
                    <div
                        class="border-t border-neutral-800 pt-4 flex justify-end gap-2 mt-auto"
                    >
                        <Button
                            type="button"
                            variant="secondary"
        ...
        {#if data.pagination.totalPages > 1}
            <div
                class="flex justify-center items-center gap-3 mt-8 border-t border-neutral-800 pt-6"
            >
                <a
                    href={getPageUrl(data.pagination.page - 1)}
                    class="inline-flex items-center justify-center h-10 px-4 rounded-lg border border-neutral-850 bg-neutral-900/40 text-sm font-medium transition-all duration-300 {data
                        .pagination.page <= 1
                        ? 'opacity-40 pointer-events-none'
                        : 'hover:border-neutral-700 text-neutral-200'}"
                >
                    <i class="ri-arrow-left-s-line"></i> Sebelumnya
                </a>

                <span
                    class="text-xs text-neutral-450 font-semibold bg-neutral-900 border border-neutral-800 px-3 py-1.5 rounded-md"
                >
                    Halaman {data.pagination.page} dari {data.pagination
                        .totalPages}
                </span>
